...
(function () {
  ...
  var T = {
    ...
    '4 tiles': '4 клетки',
    '5 tiles': '5 клеток',
    'Everyday rows leave octogen out; the octogen group is folded below the table. Blast is the wave radius in tiles. Knockdown is how far out a warrior stays on the floor for at least two seconds. Walls is how far a resin wall breaks: a wall takes blast at ×3.64, a door at ×7.5. Kills counts castes destroyed by a direct hit, out of seventeen. HE is what one round does to the front line, T2 and up, two or three tiles away: destroyed ☠ and crippled ▲, summed over both distances. Fire is intensity / reach in tiles / seconds. Steps is how many distinct reactions to brew: ANFO four, cyclonite six, octogen eight. Named rows are recipes from players, with every number computed here. Click a row to load it.': 'Базовые строки обходятся без октогена, октогеновая группа свёрнута под таблицей. Волна — радиус взрывной волны в клетках. Сбивает — в каком радиусе воин лежит на земле не меньше двух секунд. Стены — в каком радиусе ломается смоляная стена: стена получает от взрыва ×3.64, дверь ×7.5. Убивает — сколько каст из семнадцати гибнет от прямого попадания. Фугас — что один снаряд делает с линией фронта, T2 и выше, в двух-трёх клетках: уничтожено ☠ и в крите ▲, в сумме по обеим дистанциям. Огонь — интенсивность / радиус в клетках / секунды. Реакций — сколько разных реакций варить: АНФО четыре, циклонит шесть, октоген восемь. Именные строки — рецепты игроков, все числа посчитаны здесь. Клик по строке загружает её в сборщик.',
