...
(function () {
  ...
  var T = {
    ...
    '4 tiles': '4 клетки',
    '5 tiles': '5 клеток',
    'Everyday rows leave octogen out and are judged by what a grenade does on a shift: how far out a warrior stays knocked down for two seconds, how far resin walls break, and how many T0–T1 die within three tiles. Mass-produced is the best inside the four reactions of ANFO, Assault keeps nine tenths of the peak for the least material, and the octogen-free peak is the most the casing gives without it. Blast does break resin walls: a wall takes explosions at ×3.64, so its 900 is 247 raw, and 600 for a door is only 80. A sticky charge still deletes the walls it touches whatever it holds, so the breach row is the cheapest filling that goes off. Denial ranks burning ground per unit of material, four tiles or wider. Named rows are recipes from players, with every number computed here. The octogen group is folded below the table. Material counts every unit of base chemistry the mixture costs, and Steps the distinct reactions it needs: ANFO four, cyclonite six, octogen eight. Click a row to load it.': 'Базовые строки обходятся без октогена и оцениваются по тому, что граната делает на смене: в каком радиусе воин лежит на земле не меньше двух секунд, в каком радиусе ломаются смоляные стены и сколько T0–T1 гибнет в пределах трёх клеток. «Массовая» — лучшее в пределах четырёх реакций АНФО, «Штурм» держит девять десятых пика за минимум материала, «Предел без октогена» — максимум корпуса без него. Смоляные стены взрыв ломает: стена получает от взрыва ×3.64, так что её 900 — это 247 сырого урона, а 600 у двери — всего 80. Липкий заряд по-прежнему удаляет стены, которых касается, с любой начинкой, поэтому строка пролома — самая дешёвая начинка, которая срабатывает. Отсечение ранжирует горящую землю на единицу материала, от четырёх клеток шириной. Именные строки — рецепты самих игроков, но все числа посчитаны здесь. Октогеновая группа свёрнута под таблицей. Материал считает все единицы базовой химии в смеси, Реакций — сколько разных реакций нужно провести: АНФО четыре, циклонит шесть, октоген восемь. Клик по строке загружает её в сборщик.',
