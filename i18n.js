...
(function () {
  ...
  var T = {
    ...
    '2 tiles': '2 клетки',
    '3 tiles': '3 клетки',
    'The best mixture this casing can hold for each job, found by an exhaustive sweep rather than a guess. Material counts every unit of base chemistry the mixture costs. Steps counts the distinct reactions it needs, which is the part that eats a shift: ANFO is four, cyclonite six, octogen eight, and reagents sharing a chain share its steps. The last row is the best this casing can do without a longer process than ANFO. Click a row to load it.': 'Лучшая смесь для каждой задачи в этом корпусе, найденная полным перебором, а не на глазок. Материал считает все единицы базовой химии, в которые обходится смесь. Реакций — сколько разных реакций придётся провести, и именно это съедает смену: у АНФО четыре, у циклонита шесть, у октогена восемь, а реагенты на общей цепочке делят её шаги. Последняя строка — лучшее, что корпус даёт без цепочки длиннее АНФО. Клик по строке загружает её в сборщик.',
