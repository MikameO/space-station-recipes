...
(function () {
  ...
  var T = {
    ...
    '2 tiles': '2 клетки',
    '3 tiles': '3 клетки',
    'The best mixture this casing can hold for each job, found by an exhaustive sweep rather than a guess. Material counts every unit of base chemistry the mixture costs. Steps counts the distinct reactions it needs, which is the part that eats a shift: ANFO is four, cyclonite six, octogen eight, and reagents sharing a chain share its steps. HE is what one indirect hit does to the castes that actually hold a line, T2 and up, two or three tiles away: destroyed, then crippled. A dash means the round cannot reach them at all, which is true of every grenade here. Click a row to load it.': 'Лучшая смесь для каждой задачи в этом корпусе, найденная полным перебором, а не на глазок. Материал считает все единицы базовой химии, в которые обходится смесь. Реакций — сколько разных реакций придётся провести, и именно это съедает смену: у АНФО четыре, у циклонита шесть, у октогена восемь, а реагенты на общей цепочке делят её шаги. Фугас — что одно непрямое попадание делает с кастами, которые действительно держат линию, то есть T2 и выше, в двух-трёх клетках: сначала уничтоженные, потом в крите. Прочерк значит, что боеприпас до них не достаёт вообще, и так обстоит дело со всеми гранатами здесь. Клик по строке загружает её в сборщик.',
