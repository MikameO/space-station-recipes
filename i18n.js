...
(function () {
  ...
  var T = {
    ...
    // Ordnance tab (Series O)
    'Casing': 'Корпус',
    'Blast dampener': 'Гаситель волны',
    'Mixture': 'Смесь',
    'Add a reagent…': 'Добавить реагент…',
    'Fill casing': 'Заполнить корпус',
    'Detonation': 'Подрыв',
    'Empty casing. Add a reagent to begin.': 'Корпус пуст. Добавьте реагент.',
    'Power': 'Мощность',
    'Falloff': 'Спад',
    ...
    'No coloured flame': 'Пламя без цвета',
    'Logistics cost:': 'Логистическая цена:',
    'Curves': 'Кривые',
    'Falloff bottoms out before power reaches the casing ceiling, so blast radius peaks on a cheaper mixture than the strongest one.': 'Спад упирается в нижний предел раньше, чем мощность достигает потолка корпуса, поэтому радиус волны пикует на смеси дешевле самой мощной.',
    'Mode': 'Режим',
    'Blend two reagents across a full casing': 'Доля двух реагентов в полном корпусе',
    'Sweep one reagent from empty to full': 'Свип одного реагента от нуля до полного',
    'Reagent': 'Реагент',
    'From': 'От',
    'To': 'К',
    'Mixture surface': 'Поверхность смесей',
    'View': 'Вид',
    '3D surface': '3D-поверхность',
    'Flat map': 'Плоская карта',
    'Third reagent': 'Третий реагент',
    'Supplied as': 'Как задан',
    'Fills the rest': 'Заполняет остаток',
    ...
    'Up': 'По вертикали',
    'Colour by': 'Цвет по',
    'Load this mix': 'Загрузить смесь',
    'Build to a spec': 'Собрать под задачу',
    'Ready recipes': 'Готовые рецепты',
    'What it does to them': 'Что это с ними сделает',
    'Range': 'Дистанция',
    'Point blank': 'В упор',
    '1 tile': '1 клетка',
    'In the flame, s': 'В пламени, с',
    'Burning after, s': 'Горит после, с',
    '2 tiles': '2 клетки',
    '3 tiles': '3 клетки',
    '4 tiles': '4 клетки',
    '5 tiles': '5 клеток',
    'The best mixture this casing can hold for each job, found by an exhaustive sweep rather than a guess. Material counts every unit of base chemistry the mixture costs. Steps counts the distinct reactions it needs, which is the part that eats a shift: ANFO is four, cyclonite six, octogen eight, and reagents sharing a chain share its steps. HE is what one indirect hit does to the castes that actually hold a line, T2 and up, two or three tiles away: destroyed, then crippled. A dash means the round cannot reach them at all, which is true of every grenade here. Resin walls are not on that list because no casing can break one by damage: a plain wall takes 900 and the strongest ordnance delivers 720. They fall to a sticky charge, which deletes them within a tile whatever it is filled with, so the breach row holds the cheapest filling that still goes off. Denial ranks burning ground per unit of material, four tiles or wider, because entering a flame costs a flat 45 heat whatever its intensity. Click a row to load it.': 'Лучшая смесь для каждой задачи в этом корпусе, найденная полным перебором, а не на глазок. Материал считает все единицы базовой химии, в которые обходится смесь. Реакций — сколько разных реакций придётся провести, и именно это съедает смену: у АНФО четыре, у циклонита шесть, у октогена восемь, а реагенты на общей цепочке делят её шаги. Фугас — что одно непрямое попадание делает с кастами, которые действительно держат линию, то есть T2 и выше, в двух-трёх клетках: сначала уничтоженные, потом в крите. Прочерк значит, что боеприпас до них не достаёт вообще, и так обстоит дело со всеми гранатами здесь. Резиновых стен в этом списке нет, потому что уроном их не берёт ни один корпус: обычная стена держит 900, а самый сильный боеприпас даёт 720. Они падают от липкого заряда, который удаляет их в радиусе клетки с любым содержимым, поэтому строка пролома даёт самую дешёвую начинку, которая всё ещё срабатывает. Отсечение ранжирует горящую землю на единицу материала, от четырёх клеток шириной, потому что вход в пламя стоит ровно 45 теплового урона при любой интенсивности. Клик по строке загружает её в сборщик.',
    'The roster the in-game demolitions simulator offers, showing what the mixture above leaves of each one. A xeno takes double damage from an explosion, less its explosion armour, so the figure on a card runs ahead of the epicentre damage above. Fire meets no armour at all and is counted over the window you set: nobody stands in a flame until it dies out, and the hive pats a burning xeno at ten stacks a second. Shrapnel is still left out, so the real thing is worse than shown.': 'Тот же ростер, что у внутриигрового симулятора подрывов: что останется от каждого после смеси выше. Ксенос получает от взрыва двойной урон за вычетом взрывной брони, поэтому число на карточке больше урона в эпицентре выше. Огонь броню не видит вовсе и считается за окно, которое вы задали: в пламени до его затухания никто не стоит, а улей сбивает огонь по десять стаков в секунду. Осколки всё ещё не учтены, так что в жизни будет хуже.',
    'Masks': 'Маски',
    'Masks off': 'Маски выключены',
    'Overlay masks': 'Наложить маски',
    'Intersection only': 'Только пересечение',
    '+ Add mask': '+ Добавить маску',
    'Clear masks': 'Убрать маски',
    'Load this mix': 'Загрузить смесь',
    ...
    'Show cost in': 'Показывать цену в',
    'Material ceiling': 'Потолок материала',
    'Left axis': 'Левая ось',
    ...
    'Saved': 'Экономия',
    'Blast': 'Волна',
    'Nothing to optimise for this metric.': 'По этой метрике нечего оптимизировать.',
    'Loading ordnance data…': 'Загрузка данных по взрывчатке…',
    'Failed to load ordnance data.': 'Не удалось загрузить данные по взрывчатке.',
    // Calculator
    ...
    'Plant Healing': 'Лечение растений',
    '+ none': '+ без эффекта',
    'A chemical carrying no effect of a kind fails that row — press "+ none" on it to let those through.': 'Химикат без эффекта данного вида не проходит свою строку — нажмите в ней «+ без эффекта», чтобы такие попадали в выдачу.',
    'Or clear the search, chips, or sidebar filters.': 'Либо очистите поиск, чипы или фильтры сайдбара.',
    'Also keep chemicals that carry no effect of this kind at all — how you ask for a mutagen that simply leaves the plant alone': 'Добавить к выдаче химикаты, у которых такого эффекта нет вовсе — так ищется мутаген, который растение не трогает',
    ...
    'Value ranges': 'Диапазоны значений',
    'Tick an effect to require it, then narrow its range; clear a bound to drop that side of the inequality. Several ticked effects must all match. Press "+ none" on a row to also keep chemicals that carry no such effect at all — that is how you ask for a mutagen that leaves the plant alone.': 'Отметьте эффект, чтобы он стал обязательным, и сузьте диапазон; очистите границу, чтобы снять эту сторону неравенства. Несколько отмеченных эффектов должны совпасть все сразу. Кнопка «+ без эффекта» в строке добавляет к выдаче химикаты, у которых такого эффекта нет вовсе — так и ищется мутаген, который растение не трогает.',
    'Range present in the data': 'Диапазон, встречающийся в данных',
    'Affects plants in a hydroponics tray — also matches the Botany category filter': 'Действует на растения в гидропонике — попадает также под фильтр категории Botany',
    ...
    // Botany D7 — random mutations + mechanic limits (botany.js)
    '\u{1F9EC} Random mutations': '\u{1F9EC} Случайные мутации',
    '\u2699 Mechanic limits': '\u2699 Потолки механик',
    'What a mutagen can roll, and how likely each roll is. Odds are per 15-second growth cycle at the dose in the column header. The table follows the Source filter, because forks run different botany code.': 'Что может выпасть от мутагена и с какой вероятностью. Шанс — за один 15-секундный цикл роста при дозе из заголовка колонки. Таблица следует за фильтром «Источник»: форки крутят разный код ботаники.',
    'Random mutations explained': 'Пояснение таблицы мутаций',
    "The caps that decide how far potency, health and yield can go. These constants live in the game's C# code, not in extractable YAML, so this block is curated — see Sources for the files and the date.": 'Потолки, которые решают, как далеко уедут потенция, здоровье и урожай. Эти константы лежат в C#-коде игры, а не в извлекаемом YAML, поэтому блок курируемый — файлы и дату сверки смотрите в источниках.',
    'Mechanic limits explained': 'Пояснение потолков механик',
    'All forks': 'Все форки',
    'What it changes': 'Что меняет',
    'Step': 'Шаг',
    'Reversible': 'Обратима',
    'rungs': 'ступеней',
    'flips': 'переключает',
    'yes': 'да',
    'plant only': 'только растение',
    'mutations': 'мутаций',
    'Mutations': 'Мутаций',
    'ladder (pre-refactor)': 'лестница (до рефактора)',
    'range (current)': 'диапазон (актуальная)',
    'Fork': 'Форк',
    'Model': 'Модель',
    "Differences from its era's baseline": 'Отличия от базы своей эпохи',
    'Source is set to All, so this is vanilla — pick a fork to see its own table.': 'Источник — «Все», поэтому показана ваниль; выберите форк, чтобы увидеть его таблицу.',
    'Names that only differ by era are matched up first, so what is listed here is a real difference in content, not a rename.': 'Имена, различающиеся только эпохой, сопоставляются заранее — здесь перечислены настоящие различия в содержании, а не переименования.',
    'Which botany rewrite this fork runs. It is read off the shape of its own mutation file, not from a list.': 'Какую версию ботаники крутит форк. Определяется по форме его собственного файла мутаций, а не по списку.',
    'The effect is a toggle: rolling it again takes it back off': 'Эффект-переключатель: повторное выпадение снимает его обратно',
    'Plant only — the produce does not carry it': 'Только растение — на плод не переходит',
    'Raises potency with no upper limit — vanilla has no such reagent': 'Поднимает потенцию без верхнего предела — в ванили такого реагента нет',
    // Mutation targets
    'Potency': 'Потенция',
    'Yield': 'Урожай',
    'Lifespan': 'Срок жизни',
    'Maturation': 'Созревание',
    'Production': 'Плодоношение',
    'Endurance (max health)': 'Выносливость (макс. здоровье)',
    'Water use': 'Расход воды',
    'Nutrient use': 'Расход питания',
    'Toxin tolerance': 'Устойчивость к токсинам',
    'Pest tolerance': 'Устойчивость к вредителям',
    'Weed tolerance': 'Устойчивость к сорнякам',
    'Low-pressure tolerance': 'Устойчивость к низкому давлению',
    'High-pressure tolerance': 'Устойчивость к высокому давлению',
    'Low-heat tolerance': 'Устойчивость к холоду',
    'High-heat tolerance': 'Устойчивость к жаре',
    'Heat tolerance': 'Температурный допуск',
    'Ideal temperature': 'Идеальная температура',
    'Viable seed': 'Всхожесть семян',
    'Seedless': 'Без семян',
    'Woody stem': 'Одревеснение',
    'Turns into kudzu': 'Превращается в кудзу',
    'Screaming': 'Крик',
    'while it is off the plant dies of unviable genetics': 'пока снята, растение гибнет от нежизнеспособной генетики',
    'no seeds can be taken from the produce': 'семена из плода уже не достать',
    'needs a sharp tool to harvest': 'для сбора нужен острый инструмент',
    'the tray spawns kudzu once weeds build up': 'при сорняках лоток порождает кудзу',
    // Mutation effects with no stat behind them
    'The plant wakes up as a ghost role': 'Растение оживает как ghost-роль',
    'Produce turns slippery': 'Плод становится скользким',
    'Produce glows': 'Плод светится',
    'Jumps to another species': 'Переходит в другой вид',
    'Rolls an extra chemical into the produce': 'Добавляет в плод случайный химикат',
    'Changes the gas it breathes out': 'Меняет выделяемый газ',
    'Changes the gas it needs': 'Меняет потребляемый газ',
    'Upgrades harvest: once, then repeating, then self-harvesting': 'Повышает тип сбора: разовый → многократный → самосбор',
    // Curated mechanic limits (config.py BOTANY_MECHANICS)
    'Mechanic limits — what stops potency, health and yield': 'Потолки механик — что останавливает потенцию, здоровье и урожай',
    'Read off upstream C# by hand — these constants are in code, not in extractable YAML. See sources for the files and the date.': 'Сверено вручную по C#-коду апстрима — эти константы лежат в коде, а не в извлекаемом YAML. Файлы и дата — в источниках.',
    'Robust Harvest stops at 50': 'Robust Harvest упирается в 50',
    'Each tick adds 3 potency, but only up to 50 — past that the chemical does nothing for potency. Crossing 30 makes the plant seedless, so take your seeds before then or restore them later with Sedin (-3 potency a unit). At 50 and above every further dose instead has a 10% chance to drop yield by 1, down to a floor of 1: keep feeding a maxed plant and you are trading harvest size for nothing.': 'Каждый тик добавляет 3 потенции, но только до 50 — дальше химикат для потенции бесполезен. Выше 30 растение становится бессемянным, так что семена снимайте заранее либо возвращайте Sedin (−3 потенции за единицу). При 50 и выше каждая следующая доза вместо этого с шансом 10% срезает урожай на 1, вплоть до 1: продолжая кормить упёршееся растение, вы меняете размер урожая ни на что.',
    'Dosing: one unit is one tick': 'Дозировка: одна единица — один тик',
    'The tray metabolises every 3 seconds and consumes 1 unit of each reagent per tick; the effect fires once and does NOT scale with how much you poured. Growth and mutation run on a slower 15-second cycle. So the amount in the tray sets the severity of the next mutation roll: N units of mutagen ≈ severity N. Severity caps at 25, and the mutation multiplier (Left4Zed, uranium) caps at 3.': 'Лоток метаболизирует раз в 3 секунды и тратит по 1 единице каждого реагента за тик; эффект срабатывает один раз и НЕ масштабируется от налитого объёма. Рост и мутации идут по более медленному 15-секундному циклу. Поэтому объём в лотке задаёт тяжесть следующего броска мутаций: N единиц мутагена ≈ тяжесть N. Тяжесть упирается в 25, множитель мутаций (Left4Zed, уран) — в 3.',
    'Each mutation rolls once per growth cycle at min(odds × severity, 1) — at severity 25 anything above 4% odds is a coin flip you lose.': 'Каждая мутация бросается раз за цикл роста с шансом min(шанс × тяжесть, 1) — при тяжести 25 всё, что выше 4%, превращается в монетку не в вашу пользу.',
    'The check repeats while odds × severity exceeds the triggers already fired, so at high severity a mutation does not just become likely, it fires several times in one cycle.': 'Проверка повторяется, пока шанс × тяжесть превышает число уже сработавших срабатываний, так что при высокой тяжести мутация не просто становится вероятной — она срабатывает несколько раз за цикл.',
    'How a potency roll moves': 'Как двигается бросок потенции',
    'Potency mutations do not drift upward; they are pulled toward the middle of their range and pushed back from the top.': 'Мутации потенции не дрейфуют вверх: их тянет к середине диапазона и отталкивает от верхней границы.',
    'The value snaps onto six rungs — 30, 44, 58, 72, 86, 100 — and steps one rung at a time, up with probability 1 − n/5 (n = current rung). From 86 that is a 20% chance of reaching 100, and at 100 the next potency roll always steps back down to 86. Lock a good roll in by harvesting and extracting seeds before the next cycle.': 'Значение защёлкивается на шести ступенях — 30, 44, 58, 72, 86, 100 — и шагает по одной, вверх с вероятностью 1 − n/5 (n — текущая ступень). С 86 это 20% шанс дойти до 100, а на 100 следующий бросок потенции всегда отбрасывает обратно на 86. Удачный бросок фиксируют урожаем и семенами до следующего цикла.',
    'The value moves ±15 with probability 1 − (P − 30)/70 of going up, so it settles around 65 and always steps down once it is at or above 100.': 'Значение ходит на ±15 с вероятностью подъёма 1 − (P − 30)/70, поэтому оседает около 65 и всегда идёт вниз, достигнув 100.',
    'Potency stops paying': 'Потенция перестаёт окупаться',
    'Produce carries clamp(Min + Potency / PotencyDivisor, Min, Max) of each chemical, so a plant saturates at (Max − Min) × PotencyDivisor potency and everything above that is wasted — Ambrosia Deus tops out at 70, poppy at 95. Two things keep scaling without a cap: gases the plant exudes, and composting the produce back into a tray (potency / 2.5 nutrients).': 'Плод несёт clamp(Min + Потенция / PotencyDivisor, Min, Max) каждого химиката, поэтому растение насыщается на потенции (Max − Min) × PotencyDivisor, и всё выше пропадает впустую — Ambrosia Deus упирается в 70, мак в 95. Без потолка растут только две вещи: выделяемые газы и компост из плода обратно в лоток (потенция / 2.5 питания).',
    'Health, endurance, old age': 'Здоровье, выносливость, старость',
    "A watered and fed plant (water above 10, nutrition above 5) regenerates about 1 health a cycle — less than a single mutagen tick costs. Ammonia adds 0.5 health per tick, diethylamine 0.1 plus a 10% chance of +1 endurance AND +1 lifespan per tick, with no upper limit: it is the only uncapped way to raise the health ceiling. Cryoxadone rolls the plant's age back, which is what keeps a repeat-harvest plant alive past its lifespan (old age costs 3-5 health a cycle).": 'Политое и накормленное растение (вода выше 10, питание выше 5) отыгрывает около 1 здоровья за цикл — меньше, чем стоит один тик мутагена. Аммиак даёт 0,5 здоровья за тик, диэтиламин — 0,1 плюс 10% шанс +1 выносливости И +1 срока жизни за тик, без верхнего предела: это единственный безлимитный способ поднять потолок здоровья. Криоксадон откатывает возраст — именно он держит многолетнее растение живым после срока жизни (старость стоит 3–5 здоровья за цикл).',
    'Pump endurance only after the mutagen is gone: an endurance mutation snaps the stat back inside 50…150 and throws the diethylamine work away.': 'Качайте выносливость только после того, как мутаген убран: мутация выносливости защёлкнет её обратно в 50…150 и обнулит работу диэтиламина.',
  ...
  var RX = [
    ...
    [/^(\d+) value conditions? active, and every one must match\.$/, function (m) {
      return 'Активных условий по значению: ' + m[1] + ' — совпасть должны все.';
    }],
