...
(function () {
  ...
  var T = {
    ...
    '2 tiles': '2 клетки',
    '3 tiles': '3 клетки',
    'The best mixture this casing can hold for each job, found by an exhaustive sweep rather than a guess. Work counts every unit of base chemistry the mixture costs, which phoron alone hides: octogen is light on phoron and needs frost oil out of a garden. A row marked ▲ needs something no dispenser hands out. The last two rows skip those entirely. Click a row to load it.': 'Лучшая смесь для каждой задачи в этом корпусе, найденная полным перебором, а не на глазок. Труд считает все единицы базовой химии, в которые обходится смесь, а форон это прячет: октоген по форону дешёв, а требует фрост-ойла с грядки. Строка со знаком ▲ требует того, чего раздатчик не выдаёт. Последние две строки обходятся без них вовсе. Клик по строке загружает её в сборщик.',
