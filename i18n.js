...
(function () {
  ...
  var T = {
    ...
    'Tick an effect to constrain its value. Carrying no effect of that kind counts as 0, so "plant health ≥ 0" also admits chemicals that leave the plant alone. Several ticked effects must all match — that is how you ask for a mutagen that does not poison the plant.': 'Отметьте эффект, чтобы ограничить его значение. Отсутствие эффекта считается нулём — поэтому «здоровье растения ≥ 0» пропускает и химикаты, которые растение не трогают. Несколько отмеченных эффектов должны совпасть все сразу — так и ищется мутаген, который не травит растение.',
