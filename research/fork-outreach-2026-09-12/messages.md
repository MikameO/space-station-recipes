...
Плейсхолдеры: `{FORK}` имя форка как его называют игроки, `{LINK}` ссылка из [tracking.md](tracking.md), `{REAGENTS}` / `{REACTIONS}` цифры из [contacts.md](contacts.md), `{PARENT}` родительский форк для наследников (Funky → Goob, Monolith → Frontier, ADT → Corvax, Fish → Sunrise), `{MAPS}` число карт, если для форка они запечены.
...
- Не обещать фичи, которых нет (Ordnance и Библиотека документов не упоминаются: они новые и не проверены аудиторией).
