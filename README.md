...
| **Tactical** | Marine tactical map | RMC14, Space Stories, CMU and RuCM planets as the in-game tacmap: click a tile for its coordinates, calibrate once per round against one lased tile (the planet shifts ±500 each round), a mortar helper (rings, aim error, dial, shot timers, hit zone), OB and supply-drop permissions, markers, lines and areas shared as chat text, a grid in in-game numbers, variable map areas with their odds, landmarks with sprites up close. Separate page: [tactical.html](https://mikameo.github.io/space-station-recipes/tactical.html) |
| **Tablet** | Officers' room | A per-round room on the tactical map. Stage 1: a staff officer sends the mortar crew strike and position requests; the crew answers with statuses and the officer sees the mortar and its range. Off on every server until its administration sanctions it; nothing is read from the game |
