...
**Addendum, same day.** The reagent the player actually meant was not butter. `BASE_DISPENSER_CHEMICALS` in `config.py` lists twenty-five reagents; upstream's `ChemDispenser` spawns with twenty jugs (`Entities/Structures/Dispensers/chem.yml`, `EntityTableContainerFill`, verified 2026-09-12). The extra five are Plasma, Silver, Water, WeldingFuel and Oil — and upstream has no jug entity for Plasma or Oil at all, while Water, Silver and WeldingFuel do have one in chem storage. Oil is brewed or ground from crudcoa, so flagging it a dispenser chemical both hid its route and stopped the plan's tree there; the Med-Chem Starter preset asks for 4u of it. The planner therefore carries the verified twenty itself (the same pattern as the container capacities above) and labels the rest honestly, without touching `baseChemicals`: whether Oil should expand into its recipe is a data decision with app-wide effects — `Water`'s own `r.recipe` is «20x Blood», so a naive un-flagging would have the calculator propose brewing water out of blood. Filed as R8 with the question put to the owner, and the two meanings (`isDispenser` = literally in those twenty; «stop the tree here» = trivially at hand) marked for separation.
