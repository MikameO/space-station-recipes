...
**Status:** принято war-room; ждёт ответов владельца по пяти пунктам раздела «Needs Human Judgment», после них статус «accepted» и правки спеки [docs/design/2026-09-13-tactical-tablet.md](../design/2026-09-13-tactical-tablet.md)
...
**Chosen**: один DO на комнату, журнал `ops(seq…)` в SQLite объекта; клиент опрашивает `GET /room/<code>/ops?since=<seq>` с секретом в заголовке `X-Room-Session`, сервер отвечает `Retry-After: 2` при живом запросе и `Retry-After: 10` в покое. Каждый кадр несёт `serverNow`; клиент держит смещение часов, измеренное на `hello` как RTT/2, и рисует `deadlineAt − serverNowEst`. Переходы состояний запроса — CAS по `expectedStatus`; отказ возвращает `409 {status}` и не пишет строк. Повторяющихся таймеров нет: сервер отдаёт `deadlineAt`, один alarm ставится однократно на срок жизни комнаты. WebSocket — backlog, открывается при третьей одновременной комнате.
...
Пять пунктов, которые эксперты не могут закрыть за владельца; до ответов статус решения не переходит в «accepted»:

1. **Условие (б) исключения** — «в комнату попадает только то, что уже сказано по внутриигровой связи». Domain: без него санкции не будет; UX: реализуется подписью «не передано» без чекбокса и без замедления. Принимаешь ли это условие как рамку продукта?
2. **Когорта пилота** — три именованных офицера пилотного сервера письменно, владелец в тройку не входит; нет трёх имён за 7 дней — сборка дальше V4 не идёт. Реалистично ли собрать три имени вне своего круга?
3. **Ключ у администрации** — Decision 3 предлагает, чтобы `keyId` держала администрация Stories (глава администрации лично) с именной ссылкой «отключить нас»; если откажутся — ключ у нас. Согласен ли предложить это в письме?
4. **Экспорт `view` из серии T** — шов требует одной строки в `tactical.js` и контракта «слой переживает перерисовку» от параллельной сессии; при отказе — форк 60 строк. Кто из двух сессий делает шов и когда?
5. **Заморозка** — нет письменного «да» за три недели после письма → серия замораживается на V3, ресурсы уходят в Q и J. Принимаешь ли это правило заранее?
