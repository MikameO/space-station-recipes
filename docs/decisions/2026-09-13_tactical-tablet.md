...
**Chosen**: `INSERT ops` + `UPSERT objects` без дополнительных индексов = ≤2 строки на операцию → 8 комнат в сутки на Free. Потолок читается при создании комнаты: `ROOMS_MAX_CONCURRENT=3`, `ROOMS_MAX_DAILY=6`, реестровый DO ведёт счётчики и `GET /health` (только числа, никогда список). Лимитер считает по источнику входа (IP-адрес из `CF-Connecting-IP`), никогда по коду комнаты. Байтовый бюджет комнаты 2 МБ и 2000 объектов — исполнимая форма потолка; снимок отдаётся пачками по 200. Деградация: при потолке отказывают **новые** комнаты текстом, активная комната не трогается никогда; жёсткий отказ (бюджет, kill) переводит её в «Радиомолчание» — баннер над картой и «данные заморожены с 14:32» на каждой карточке.
...
**Chosen**: карточка огня серии T (~180 px) монтируется внутрь раскрытого запроса, отдельного переключения «Комната → Огонь» для исполнения нет. Состояния запроса: `requested → accepted → loaded → firing → done | denied`, переходы через `expectedStatus`; новое значение `loaded` даёт ОБ две строки владельцев — «Заряжено» (OT) и «Выстрел» (SO), автор строки берётся из автора операции, новых скаляров нет. Маяк поставки — элемент массива `flags: ["beacon"]` на запросе, чтобы следующий признак не требовал миграции снимка; всё прочее («чем заряжено», примечания) — в `notes`. Отозванный штабом запрос показывает «отозвано штабом», а не исчезает. Под каждой новой меткой противника подпись «не передано», которую снимает одним кликом любой в комнате. Ресурсы — `claimable`: подтверждённый участник берёт «Расчёт» на конкретный миномёт, и право `acceptRequest` действует только на этот ресурс. Звук (владелец, 2026-09-14, пункт 7 «Owner overrides»): расчёт, на чей ресурс нацелен запрос, слышит повторяющийся пинг каждые 10 с, срочный — каждые 5 с, пока запрос не обработан или не отмечен «Услышал» локально; автор ничего не слышит; пока комната заморожена (радиомолчание, рубильник администрации, бюджет), заблокирована или закрыта, пинг молчит и звучит снова сразу после.
...
«Радиомолчание»: жёсткий отказ или рубильник администрации переводит комнату в чтение с баннером и отметкой заморозки на каждой карточке; стоп-флаг доходит примерно за минуту (комната перечитывает его на первом запросе не раньше чем через 60 с после прошлой проверки) и на ближайшем alarm'е.

Конец раунда: 8 минут простоя → плашка «новый раунд?»; блокировка запрещает операции, экспорт (`JSON` + текстовая хронология по должностям) живёт час, затем alarm удаляет DO. Клиентские ключи — `chemdb-tactical:room/<code>:<client>` (состояние комнаты), `chemdb-tactical:room-current:<client>` (какую комнату восстановить) и `chemdb-tactical:room-client` (id браузера), настройки звука — `chemdb-tactical:room-prefs`, синхронизация вкладок событием `storage`.
...
| R8 звук планшета глушит 7-секундное предупреждение об ударе | medium | повторяющийся пинг только расчёту, на чей ресурс нацелен запрос (каждые 10 с, срочный — 5 с), до обработки запроса или «Услышал»; автор не слышит; молчит, пока комната заморожена, заблокирована или закрыта; громкость и выключатель — лично | ACCEPTED |
...
5. **Этап 1 — офицер штаба и миномётный расчёт (владелец, 2026-09-13, после плана):** «можно начать с малой версии для двоих… Этого будет достаточно». Первый выпуск — комната на двоих: офицер штаба выдаёт расчёту запросы на удар и на позицию, расчёт видит их на карте, отвечает статусами и одним нажатием переносит позицию на панель «Огонь», офицер видит миномёт и его дальности. Поставляемая политика содержит должности `so`, `mortar`, `observer` и один миномёт; полная политика остаётся тестовой фикстурой `scripts/fixtures/room_policy_full.json`, так что логика и Worker не сужаются. Новый тип запроса `position`; автор запроса никогда не принимает и не выполняет его сам. V8 (общие метки, линии, зоны) отложен до полной версии. «Полезная» комната для KT-B на этапе 1 — два подтверждённых участника, четыре рабочие операции и хотя бы один выполненный запрос (заменено пунктом 6, «KT-B на этапе 1»: подтверждены обе должности, `so` и `mortar`, и хотя бы один запрос выполнен должностью расчёта, а не автором; пороги четырёх рабочих операций и двух клиентов остаются). Ворота санкции не меняются. Правки задач — раздел «Stage 1» плана.

6. **Правила этапа 1 по итогам ревью (контроллер, 2026-09-14; владелец может переопределить):**
   - **Кнопки автора запроса.** Офицер, выдавший запрос, на своём запросе видит только «Снять»: до принятия это отмена, после принятия — отзыв без выбора причины, в карточке «отозвано штабом». «Принять», «Отклонить», «Взять цель», «Встать сюда» и «Выполнено» получает любой участник с правом `acceptRequest`, кроме автора: владелец ресурса, занявший расчёт или штаб, в том числе второй офицер штаба; на этапе 1 это обычно расчёт.
...
7. **Сигнал о новом запросе (владелец, 2026-09-14):** «при появлении запросов для какого-то отделения — хорошо, чтобы воспроизводился какой-то пинг, который бы привлекал внимание, но при этом был хорошо слышен в случае, если проходит активный бой — например, он также может издавать звук регулярно, пока не будет отмечен». Решение: расчёт, на чей ресурс нацелен запрос, слышит повторяющийся пинг каждые 10 с (срочный — каждые 5 с), пока запрос не обработан или не отмечен «Услышал» локально; автор не слышит ничего; пока комната заморожена (радиомолчание, рубильник администрации, бюджет), заблокирована или закрыта, пинг молчит и звучит снова сразу после; подробности — спека, раздел «Запросы и доска ресурсов».
8. **Тариф на старте (владелец, 2026-09-14):** «Пока работаем на бесплатном тарифе. Далее сам переключу на более дорогой.» Пункт 2 откладывается: Worker работает на бесплатном тарифе Workers, потолки возвращены к расчёту Decision 4 — `ROOMS_MAX_CONCURRENT=3`, `ROOMS_MAX_DAILY=6` (`worker/wrangler.toml`). При переходе на Workers Paid владелец поднимает их до 6 и 30 и передеплоивает Worker.
...
## Stage 2a (2026-09-14, решения владельца в чате)

Спека: [../design/2026-09-14-tactical-tablet-stage2a.md](../design/2026-09-14-tactical-tablet-stage2a.md). Цель владельца — полевой тест на продакшн-версии сайта вечером того же дня с миномётным расчётом и, возможно, вторым офицером.

1. Адресат запроса — любая должность (с отрядом) или конкретный участник, поле `to` задаётся при создании. Принимают адресат и штаб; владелец ресурса и закреплённый расчёт на адресный запрос не действуют. «Удар миномёта» адресуется только расчёту.
2. Позицию передаёт любой подтверждённый участник (`member.pos`, сервер ставит `posAt`); у миномётного расчёта та же отправка разворачивает его миномёт на тайле. Запись `pos` и `cal` — не пульс: блокировка простоя и радиомолчание её отклоняют, пульсом остаётся только запись с одним `presentAt`.
3. Одна привязка комнаты, свежая публикация заменяет прежнюю (`calibration` с `offset`, `tile`, `reading`, согласованными между собой). У каждого — «Применить у себя», «Опубликовать мою привязку» (уточнение своим замером) или «Оставить свою»; `member.cal` показывает в реестре, кто работает по какой.
4. Новый тип запроса `task` («Задача»): адресат и текст обязательны, точка на карте необязательна; действия — принять или отказать, выполнено или отказ, повторить.
5. Блок калибровки панели огня сворачивается в строку, когда привязка готова и проверена.
6. Процесс. Владелец: «Делай дальше сам целиком», «Всё по рекомендациям — до конца». War-room по изменению контракта (Tier 2) не проводился из-за срока полевого теста; его заменяет финальное адверсариальное ревью диффа перед деплоем.
7. Предупреждение владельцу, 2026-09-14: игра в живом раунде Space Stories с комнатой без письменного «да» администрации противоречит правилу 3 сервера и VISION §4.3. Публичное включение (`ROOM_URL`, статус `pilot`) не делается; тестеры включают скрытую комнату у себя dev-флагом, решение об игре в живом раунде остаётся за владельцем.
