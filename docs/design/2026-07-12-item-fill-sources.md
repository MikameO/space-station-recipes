...
## Не-цели (YAGNI) и follow-ups, найденные при реализации
...
- **StorageFill-распаковка** (найдено в D3c): паки таблеток форков (ADT `*Pack`) — это коробки
  с `StorageFill`, а не растворы; нужен рекурсивный канал «контейнер → предметы → заливки».
- **Патченные ванильные копии вендоров**: форки (Sunrise, Corvax, частично Goob/DeltaV)
  добавляют свои бутылки в свои копии ванильного `boozeomat.yml` — нужен harvest-паттерн
  Phase 2b для channel-файлов.
- Расширить `scripts/audit_fork_manifests.py` на новые манифест-ключи.
- Косметика: в reason карточки предпочитать метку без тега «(hacked)», если есть обычная
  (сейчас алфавитная сортировка ставит tagged-метку первой у Mayo).
