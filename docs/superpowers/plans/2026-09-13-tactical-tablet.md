...
```
   ...
   with every hard-coded threshold in the KT-B lines and in `passed` read from `STAGES[args.stage]`. Step 2 expectation for the two-officer end-to-end export: `useful=True` if both requests were done, otherwise `useful=False`, and `KT-B NOT MET` either way (one room). *Superseded by «KT-B» in «Fix round after the code reviews» below:* `'confirmed': 2` in the listing above became the required posts `so` and `mortar` (`'posts': ['so', 'mortar']`), and one request done by the mortar crew post, not by its author, is enough; the ops and clients thresholds stay 4 and 2. The end-to-end export is `useful=True` once the crew has done either request, and `KT-B NOT MET` still (one room).
